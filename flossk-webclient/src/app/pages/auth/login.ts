...
@Component({
                ...
                <div class="w-full max-w-md">
                    ...
                    <div class="hidden lg:block text-center mb-10">
                        <h2 class="text-2xl font-bold text-surface-900 dark:text-surface-0">
                            {{ isLoginMode ? (isMfaRequired ? 'Two-factor authentication' : 'Welcome back') : isForgotMode ? 'Reset password' : 'Create account' }}
                        </h2>
                        <p class="text-surface-500 dark:text-surface-400 mt-1">
                            {{ isLoginMode ? (isMfaRequired ? 'Verify your identity' : 'Log in to your account') : isForgotMode ? 'Enter your email to receive a reset link' : 'Sign up for a new account' }}
                        </p>
                    </div>
                    ...
                    @if (isLoginMode && !isMfaRequired) {
                        ...
                        <div class="flex flex-col items-center gap-4">
                            @if (selectedMfaMethod === null) {
                                <p class="text-muted-color text-sm text-center mb-2">Choose a verification method:</p>
                                <p-button label="Authenticator App" icon="pi pi-mobile" styleClass="w-full" (onClick)="selectedMfaMethod = 'totp'"></p-button>
                                <p-button label="Recovery Code" icon="pi pi-key" styleClass="w-full" severity="secondary" (onClick)="selectedMfaMethod = 'recovery'"></p-button>
                                @if (passkeySupported) {
                                    <p-button label="Passkey" icon="pi pi-lock" styleClass="w-full" severity="secondary" [loading]="isUsingPasskey" (onClick)="onUsePasskey()"></p-button>
                                }
                            } @else if (selectedMfaMethod === 'totp') {
                                <div class="w-full flex flex-col items-center gap-3">
                                    <p class="text-muted-color text-sm text-center">Open your authenticator app and enter the 6-digit code.</p>
                                    ...
                                    <span class="text-sm font-medium cursor-pointer text-primary hover:underline" (click)="selectedMfaMethod = null; mfaCode = ''">Back to options</span>
                                </div>
                            } @else if (selectedMfaMethod === 'recovery') {
                                <div class="w-full flex flex-col items-center gap-3">
                                    <p class="text-muted-color text-sm text-center">Enter one of your recovery codes.</p>
                                    <input pInputText id="recoveryCode" placeholder="Recovery code" class="w-full" [(ngModel)]="recoveryCode" />
                                    <p-button label="Verify" styleClass="w-full" [loading]="authService.isLoading()" (onClick)="onVerifyMfa()"></p-button>
                                    <span class="text-sm font-medium cursor-pointer text-primary hover:underline" (click)="selectedMfaMethod = null; recoveryCode = ''">Back to options</span>
                                </div>
                            }
                            @if (selectedMfaMethod === null) {
                                <div class="text-center mt-2">
                                    <span class="font-medium cursor-pointer text-primary hover:underline" (click)="cancelMfa()">Cancel</span>
                                </div>
                            }
                        </div>

                        <div class="fixed inset-0 bg-black/60 z-50 items-center justify-center" [style.display]="selectedMfaMethod === 'passkey' ? 'flex' : 'none'" (click)="cancelPasskeyAuth()">
                            <div class="bg-surface-0 dark:bg-surface-900 rounded-2xl p-12 flex flex-col items-center gap-6" (click)="$event.stopPropagation()">
                                <svg #crowSvg class="w-40 h-auto" viewBox="730 540 120 120" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path #crowPath d="M805,594.7v4.74L770.67,586l-3.92,13.46H765L762,584a30.91,30.91,0,0,1-9.24-7.84,22.08,22.08,0,0,1-4.58-11.65c-.8-8.09-7.92-8.74-7.92-8.74V554.4s12.78-11.24,21.19-.69a78.51,78.51,0,0,0,5.83,6.63s21.11,19.27,24.71,22.73Z" fill="none" stroke="var(--primary-color)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
                                    <path #beakTop d="M805,594.7v4.74L770.67,586" fill="none" stroke="var(--primary-color)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" opacity="0" style="transition: opacity 0.3s"/>
                                </svg>
                                <p class="text-sm text-muted-color">Authenticating with passkey...</p>
                                <p-button label="Cancel" severity="secondary" [text]="true" (onClick)="cancelPasskeyAuth()"></p-button>
                            </div>
                        </div>
    ...
    styles: [`
        ...
        .crow-animating {
            animation: crow-draw 1.2s ease-out forwards, crow-fade 0.4s ease-out 1.6s forwards;
        }
        .beak-open {
            transform-origin: 770px 586px;
            animation: beak-open 0.3s ease-out 2.0s forwards;
        }
        @keyframes beak-open {
...
export class Login implements AfterViewInit {
    ...
    @ViewChild('crowSvg') crowSvg!: ElementRef<SVGSVGElement>;
    @ViewChild('crowPath') crowPath!: ElementRef<SVGPathElement>;
    @ViewChild('beakTop') beakTop!: ElementRef<SVGPathElement>;
    ...
    selectedMfaMethod: 'totp' | 'recovery' | 'passkey' | null = null;
    ...
    private playCrowAnimation(retries = 5) {
        const path = this.crowPath?.nativeElement as SVGPathElement | null;
        if (!path) {
            if (retries > 0) {
                setTimeout(() => this.playCrowAnimation(retries - 1), 100);
            }
            return;
        }
        const length = path.getTotalLength();
        ...
        const animate = (timestamp: number) => {
            ...
            if (progress < 1) {
                ...
                setTimeout(() => {
                    path.style.strokeDashoffset = `${length}`;
                    startTime = null;
                    const animate2 = (timestamp2: number) => {
                        if (!startTime) startTime = timestamp2;
                        const elapsed2 = timestamp2 - startTime;
                        const progress2 = Math.min(elapsed2 / duration, 1);
                        path.style.strokeDashoffset = `${length * (1 - progress2)}`;
                        if (progress2 < 1) {
                            this.animationFrameId = requestAnimationFrame(animate2);
                        } else {
                            const beak = this.beakTop?.nativeElement as SVGPathElement | null;
                            if (beak) {
                                beak.style.opacity = '1';
                                beak.style.transition = 'transform 0.3s ease-out';
                                beak.style.transform = 'rotate(15deg)';
                                beak.style.transformOrigin = '770px 586px';
                            }
                            setTimeout(() => {
                                if (beak) {
                                    beak.style.transition = 'transform 0.3s ease-out, opacity 0.3s ease-out';
                                    beak.style.transform = 'rotate(0deg)';
    ...
    async onUsePasskey() {
        if (!this.passkeySupported) {
            this.authService.error.set('Passkeys are not supported in this browser.');
            return;
        }

        this.selectedMfaMethod = 'passkey';
        this.isUsingPasskey = true;
        this.authService.error.set(null);

        this.playCrowAnimation();

        this.http.post(`${environment.apiUrl}/Mfa/passkeys/authenticate-start?userId=${this.mfaUserId}`, {}).subscribe({
                ...
                try {
                    const publicKey: PublicKeyCredentialRequestOptions = {
                        ...
                        allowCredentials: options.allowCredentials.map((c: any) => ({
                            id: Uint8Array.from(atob(this.base64UrlToBase64(c.id)), c2 => c2.charCodeAt(0)),
                    ...
                    const credentialJson = JSON.stringify({
                        ...
                        response: {
                            ...
                            authenticatorData: this.uint8ArrayToBase64(new Uint8Array(credential.response.authenticatorData)),
                            signature: this.uint8ArrayToBase64(new Uint8Array(credential.response.signature)),
                            userHandle: credential.response.userHandle ? this.uint8ArrayToBase64(new Uint8Array(credential.response.userHandle)) : null
                        }
                    });

                    this.http.post(`${environment.apiUrl}/Mfa/passkeys/authenticate-complete?userId=${this.mfaUserId}`, { credentialJson }).subscribe({
                        next: (response: any) => {
                            this.isUsingPasskey = false;
                            this.selectedMfaMethod = null;
                            if (this.animationFrameId !== null) {
                        ...
                        error: (err) => {
                            ...
                            this.authService.error.set(err.error?.message || 'Passkey authentication failed.');
                        }
                    });
                } catch (err: any) {
                    this.isUsingPasskey = false;
                    ...
                    if (err.name !== 'AbortError' && err.name !== 'NotAllowedError') {
                        this.authService.error.set('Passkey authentication failed: ' + (err.message || 'Unknown error'));
                    }
                }
            },
            error: (err) => {
                ...
                this.authService.error.set(err.error?.message || 'Failed to start passkey authentication.');
            }
        });
    }
