...
public class MfaController(IMfaService mfaService) : ControllerBase
{
    ...
    public async Task<IActionResult> RegisterPasskeyComplete([FromBody] object request)
    ...
    public async Task<IActionResult> DeletePasskey(string id)
        => await _mfaService.DeletePasskeyAsync(User.FindFirstValue(ClaimTypes.NameIdentifier), id);
    ...
    public async Task<IActionResult> AssertionComplete([FromBody] PasskeyAssertionCompleteDto request)
        => await _mfaService.AssertionCompleteAsync(request);
